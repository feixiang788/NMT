...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        packed_enc_hiddens, (last_hidden, last_cell) = self.encoder(pack_padded_sequence(X, source_lengths))
        padded_enc_hiddens,padded_enc_hiddens_len = pad_packed_sequence(packed_enc_hiddens)
        enc_hiddens = padded_enc_hiddens.permute(1, 0, 2)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        split_Y = torch.split(Y, 1, 0)
        for Y_t in split_Y:
            squeeze_Y_t = torch.squeeze(Y_t, 0)
            Ybar_t = torch.cat((squeeze_Y_t,o_prev), -1)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden_unsqueeze = torch.unsqueeze(dec_hidden, -1)
        e_t = torch.squeeze(torch.bmm(enc_hiddens_proj, dec_hidden_unsqueeze), -1)
        ### END YOUR CODE
        ...
        alpha_t_unsqueeze = torch.unsqueeze(alpha_t, 1)
        a_t = torch.squeeze(torch.bmm(alpha_t_unsqueeze, enc_hiddens), 1)
